 ...
 * Mintardjo Wangsawidjaja Copyright (C) 2000 Philip Kendall and Ilmari Karonen
 *
...
char *fatalErrorInSimulator =
    "%%PMARS-F-FATAL, fatal error in simulator. Please report this error.\n";
char *warriorTerminatedEndOfRound =
    "%%PMARS-I-WARTEREND, warrior %d: %s terminated - End of round %d\n";
...
char *fatalErrorInSimulator =
    "Fatal error in simulator. Please report this error.\n";
char *warriorTerminatedEndOfRound =
    "Warrior %d: %s terminated - End of round %d\n";
...
char *helpText[] = {
    "Command groups:\n",
    "Executing:    continue execute  go       quit     skip     step     "
    "thread\n",
    "Listing:      list     search\n",
    ...
    "                       clear, turn core display on/off or set -v option "
    "to nnn\n",
#endif
    ...
    "(if) expression        execute next command in chain if expression is not "
    "0\n",
    "(l)ist [range]         list core addresses\n",
    "(m)acro [name][,file]  execute macro [name] (from [file], "
    "[user]=keyboard)\n",
    "(m)acro                list macros\n",
...
#if defined(DOSGRXGRAPHX) || defined(DOSTXTGRAPHX) || defined(LINUXGRAPHX) ||  \
    defined(XWINGRAPHX)
    "(sw)itch [1|2]         switch to left (1), right (2), or other cdb "
    "panel\n",
#endif
...
char *noColorAvailable =
    "Can't allocate color, all cells allocated and no one matches\n";
char *privateMap =
    "Can't allocate color \"%s\"; switching to private colormap\n";
...
char *tooMuchStuffErr = "Lines generated by the source exceed the limit "
...
char *credits_screen2 = "Copyright (C) 1993-95 Albert Ma, Na'ndor Sieben, "
                        "Stefan Strack and Mintardjo Wangsaw\n";
...
#else
#ifdef DOSGRXGRAPHX
char *credits_screen3 = "Graphics display version by Na'ndor Sieben\n";
#else
#ifdef CURSESGRAPHX
char *credits_screen3 =
...
#else
#ifdef DOSTXTGRAPHX
char *credits_screen3 = "Textmode display version by Stefan Strack\n";
#else
#ifdef MACGRAPHX
char *credits_screen3 = "Macintosh display version by Alex MacAulay\n";
#else
#ifdef LINUXGRAPHX
char *credits_screen3 = "Linux/SVGA display version by Martin Maierhofer\n";
#else
char *credits_screen3 = "";
#endif /* LINUXGRAPHX */
#endif /* MACGRAPHX */
...
char *usage_screen =
    ...
    " [options] file1 [files ..]\n   The special file - stands for sys$input.  "
    "It must be in quotes if it is the\n   last command on the line.\n\nAll "
    "uppercase options must be in quotes.\n";
#else
#if !defined(CURSESGRAPHX) && defined(DOSTXTGRAPHX) || defined(DOSGRXGRAPHX)
char *usage_screen = "Usage:\n   pmarsv [options] file1 [files ..]\n   The "
                     "special file - stands for standard input\n\n";
#else
