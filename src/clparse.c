...
typedef enum { clp_int, clp_addr, clp_long, clp_bool, clp_str } clp_dtype_t;
...
extern char *credits_screen1, *credits_screen2, *credits_screen3, *usage_screen,
    ...
    *errorIsLocatedIn, *unknownOption, *badArgumentForSwitch,
    *optionMustBeInTheRange, *tooManyParameters, *cannotOpenParameterFile,
    *optRounds, *optEnterDebugger, *optDisabledInServerVersion, *optCoreSize,
    *optBrief, *optCycles, *optVerboseAssembly, *optProcesses, *optKotH,
    *optLength, *opt88, *optDistance, *optFixedSeries, *optFixedPosition,
...
#ifdef MACGRAPHX
void macputs(char *outputstr);
#endif

#ifdef NEW_STYLE
void strip(char *argv[]);
void print_usage(clp_opt_t clopt[]);
int clp_parse(clp_opt_t clopt[], FILE *filep);
int parse_param(int argc, char **argv);
int next_input(FILE *filep, char *inputs);
...
void print_usage(clopt) clp_opt_t clopt[];
  ...
  if (!printed_usage) {
    ...
    for (i = 0, clip = clopt; clip->storage != NULL; ++clip) {
      if (clip->description != NULL) {
        sprintf(outs, "  -%c %s %-30s", clip->word, describe[(int)clip->dtype],
                clip->description);
        errout(outs);
        if (i++ % 2)
          errout("\n");
    ...
    if ((inputs[0] == '-') && inputs[1]) { /* option ? */
      ...
      for (i = 1; inputs[i] && (bool_switch); ++i) {
        for (clip = clopt; /* find which option is this */
             (clip->storage != NULL) && (inputs[i] != clip->word); ++clip)
          ;
#if defined(XWINGRAPHX)
          ...
          if (clip->dtype == clp_bool)
            ...
            if (inputs[i + 1] != '\0') {
              ...
              if (!next_input(filep, inputs)) {
                ...
                if ((*(char **)clip->storage = (char *)malloc(
                         (strlen(inputs) + 1) * sizeof(char))) != NULL) {
                ...
                if (clip->def < (clip->min) || clip->def > (clip->max)) {
                  code = RANGE;
                  goto ERROR;
                }
              }
      ...
      if (warriors < MAXWARRIOR) {
        ...
        if (inputs[many] == '-') { /* warrior from stdin */
          ...
          *inputs = '\0';
          ...
          many = 1;
        for (; many; many--) {
          if ((warrior[warriors].fileName = (char *)malloc(
                   (strlen(inputs) + 1) * sizeof(char))) != NULL) {
...
#define OPTNUM                                                                 \
...
#ifdef PERMUTATE
  record('r', clp_int, &rounds, 0, MAXROUND, -1, optRounds);
#else
  record('r', clp_int, &rounds, 0, MAXROUND, DEFAULTROUNDS, optRounds);
#endif
#ifndef SERVER
  record('e', clp_bool, &SWITCH_e, 0, 1, 0, optEnterDebugger);
#else
  record('e', clp_bool, &SWITCH_e, 0, 1, 0, optDisabledInServerVersion);
#endif
  record('s', clp_addr, &coreSize, 1, MAXCORESIZE, DEFAULTCORESIZE,
         optCoreSize);
  record('b', clp_bool, &SWITCH_b, 0, 1, 0, optBrief);
  record('c', clp_long, &cycles, 1, MAXCYCLE, DEFAULTCYCLES, optCycles);
  ...
  record('o', clp_bool, &SWITCH_o, 0, 1, 0, optSort);
#if defined(PSPACE)
  record('S', clp_addr, &pSpaceSize, 1, MAXCORESIZE, 0, optPSpaceSize);
#endif
#ifdef PERMUTATE
  record('P', clp_bool, &SWITCH_P, 0, 1, 0, optPermutate);
#endif
#ifdef RWLIMIT
  record('R', clp_addr, &readLimit, 1, MAXCORESIZE, 0, optReadLimit);
...
#define V_MAX 994
#endif
  record('v', clp_int, &SWITCH_v, 0, V_MAX, 103, optView);
#undef V_MAX
#endif
#if defined(VMS)
  ...
  if (!result) { /* so far so good */
    {
      ...
      if (writeLimit == 0)
        writeLimit = coreSize;
      if (readLimit < 1 || writeLimit < 1 || coreSize % readLimit != 0 ||
          coreSize % writeLimit != 0) {
        print_usage(options);
        errout(badRWLimit);
        result = CLP_NOGOOD;
      }
