...
void check386(void) {
  asm pushf asm pop ax asm and ah, 01111111 b asm or ah,
      01000000 b asm push ax asm popf asm pushf asm pop ax asm and ah,
      11000000 b asm cmp ah, 01000000 b asm je is386 printf(stub386);
  Exit(NOT386);
is386:
  return;
}
...
#if defined(DOSTXTGRAPHX) || defined(DOSGRXGRAPHX) || defined(LINUXGRAPHX) ||  \
    defined(XWINGRAPHX)
void decode_vopt(option) int option;
{
...
void body() {
  ...
  for (i = 0; (i < warriors) && (errorcode == SUCCESS); i++) {
    /* Initialize energy system for each warrior */
    if (SWITCH_E) {
      warrior[i].energy = defaultEnergy;
      warrior[i].maxEnergy = defaultEnergy;
    } else {
      warrior[i].energy = -1; /* Disabled */
      warrior[i].maxEnergy = -1;
    }
...
void Exit(errorcode) int errorcode;
{
#if defined(CURSESGRAPHX)
...
#ifdef DOS16
        ((pSpace[j] = (ADDR_T far *)farcalloc(pSpaceSize, sizeof(ADDR_T))) ==
         NULL)) {
#else
